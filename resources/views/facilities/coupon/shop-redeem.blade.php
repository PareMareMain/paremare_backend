                    ...
                    <div class="page-header">
                        <div class="page-title">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active">{{__("Apply Coupon")}}</li>
                            </ol>
                        </div>
                    </div>
                </div>
                                ...
                                <div class="basic-form">
                                    <form action="{{route('coupon.onShopRedeemCreate')}}" id="myform" method="POST" enctype="multipart/form-data">
                                        ...
                                        <div class="form-group">
                                            <label>{{__("Choose User By Name")}}</label>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-2">
                                                    <button class="btn btn-primary" type="button" id="apply">{{__("Apply")}}</button>
                                                </div>
                                            </div>
                                        </div>
                                        <div id="append">
    ...
    $('#apply').on('click',function(){
        ...
        if(coupon && userId){
            $.ajax({
                    ...
                    if(response.status==true){
                        ...
                        if(response.data1.offer_type=='amount'){
                            html +=`<div class="form-group">
                                        ...
                                        <input type="text" name="amount[0]" data-key="0" data-offer="amount" data-discount="`+response.data1.discount+`" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" id="percentage_amount_0" placeholder="{{__('Enter Total Sum Amount')}}" class="form-control percentage_amount">
                                    </div>
                                    <div class="form-group">
                        ...
                        if(response.data1.offer_type=='percentage'){
                            html +=`<div class="form-group">
                                        ...
                                        <input type="text" name="amount[0]" data-key="0" data-offer="percentage" data-discount="`+response.data1.discount+`" data-discount="`+response.data1.discount+`" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" id="percentage_amount_0" placeholder="{{__('Enter Total Sum Amount')}}" class="form-control percentage_amount">
                                    </div>
                                    <div class="form-group">
                            ...
                            if(response.data1.buy_items>0){
                                ...
                                for(i=0;i<response.data1.buy_items;i++){
                                    ...
                                    html +=`<div class="form-group">
                                            ...
                                            <input type="text" name="amount[`+ i +`]" data-key="`+ i +`" onkeypress="myFunction()" data-offer="buy-get" data-discount="`+response.data1.discount+`" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" id="percentage_amount_`+i+`" placeholder="{{__('Enter Item')}} `+k+` {{__('Price')}}(AED)" class="form-control percentage_amount">
                                        </div>`;
                                }
                            }
                            if(response.data1.free_items>0){
                                ...
                                for(j=0; j < response.data1.free_items;j++){
                                    ...
                                    html +=`<div class="form-group">
                                        ...
                                        <input type="text" name="amount_free[`+ j +`]" data-key="`+ j +`" data-offer="buy-get" data-discount="`+response.data1.discount+`" id="percentage_amount_free`+ j +`" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" placeholder="{{__('Enter Item')}} `+ l +` {{__('Price')}}(AED)" class="form-control percentage_amount_free">
                                    </div>`;
                                }
                            }
                            html +=`<div class="form-group">
                ...
                error:function() {
                    Swal.fire('error',__('Something went wrong'));
                }
            });
        }
        if(!coupon && !userId){
            Swal.fire('error',__('Enter Coupon Code and Select User Name'));
        }
        if(!coupon && userId){
            Swal.fire('error',__('Enter Coupon Code'));
        }
        if(!userId && coupon){
            Swal.fire('error',__('Select User Name'));
        }
...
<script>
    ...
    $('#percentage_amount').blur(function(){
        ...
        if()
        if(amount < discount){
            Swal.fire(__('Total Sum Amount must be greater than Discount Amount'));
        }
    })  --}}
    var amounts=[];
    var free_amount=[];
...
$("#myform").validate({
    submitHandler: function (form) {
        ...
        if(key==='amount'){
            if(amount < discount){
                Swal.fire(__('Total Price Amount should be greater than Discount Amount'));
                return false;
            }
        }
