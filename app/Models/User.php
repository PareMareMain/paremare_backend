...
{
    use HasApiTokens, HasFactory, Notifiable,HasRoles,SoftDeletes,HasTranslations, Billable;
    ...
    public function IsSubscribed(){
        ...
        // $data = $this->hasOne(Wishlist::class,'vendor_id');
        if(Auth::check()){
            $data= UserSubscription::where('user_id',$this->id)->where('status', 1)->where('end_date','>',\Carbon\Carbon::now()->format('Y-m-d'))->first();
        }
        // if(Auth::guard('api')->check()){
        //     $data= UserSubscription::where('user_id',Auth::guard('api')->user()->id)->where('status', 1)->where('end_date','>',\Carbon\Carbon::now()->format('Y-m-d'))->first();
        // }
    ...
    public function getSubscriptionDetail(){
        ...
        $subscription=UserSubscription::where('user_id',$this->id)->where('status',1)->where('end_date','>=',\Carbon\Carbon::now()->format('Y-m-d'))->take(1)->first();
        return $subscription;
    }
    ...
    public function menuList(){
        return $this->hasMany(Image::class,'vendor_id')->where('type', 1)->where('status', 1);
    }
    public function coverImages(){
        return $this->hasMany(Image::class,'vendor_id')->where('type', 2)->where('status', null);
    }
    ...
    public function tags(){
        return $this->hasMany(VendorTag::class,'vendor_id');
    }

    public function favorites(){
        return $this->hasMany(UserFavorite::class,'user_id')->select('vendor_id');
    }
    ...
    public function IsFavorite(){
        ...
        // $data = $this->hasOne(Wishlist::class,'vendor_id');
        if(Auth::check()){
            $data= UserFavorite::where('vendor_id',$this->id)->where('user_id',Auth::user()->id)->first();
        }
        return $data;
    }
